...
def scan_mrr_activity(
    topology: RNBTopology,
    permutations_to_scan: list[tuple[int, ...]] | None = None,
) -> list[MRRActivity]:
    """Scan S_N permutations and classify MRR control/activity roles."""
    mrr_meta = {
        mrr_id: (stage, pair)
    ...
    all_perms = (
        list(permutations(range(topology.N_logical)))
        if permutations_to_scan is None
        else list(permutations_to_scan)
    )
    ...
    for mrr_id in sorted(mrr_meta):
        ...
        rows.append(
            MRRActivity(
                ...
                permutations_scanned=len(all_perms),
